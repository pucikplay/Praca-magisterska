...
% \subsubsection*{Rozwiązanie równania~\ref{eq:matrix}}

% \textit{Przypadek A:\ $O_1$, $O_2$ i $O_3$ nie są współliniowe.}
% Wtedy $\text{rank}(\mathbf{A}) = 3$ oraz $\text{dim}(\text{ker}(\mathbf{A})) = 1$. Ogólnym rozwiązaniem układu~\ref{eq:matrix} jest
% \begin{equation}
%     \mathbf{x} = \mathbf{x}_p + t \cdot \mathbf{x}_h
% \end{equation}
% o rzeczywistym parametrze $t$. $\mathbf{x}_p$ jest pewnym rozwiązaniem~\ref{eq:matrix}, a $\mathbf{x}_h$ jest bazą $\text{kern}(\mathbf{A})$.
% Wektory
